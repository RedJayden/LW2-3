...
namespace File
{
	...
	{
		if ( ReadSize == READ_ALL ) ReadSize = (int)File::GetSize( h , ErrorCode );
	...
	{
		HANDLE h = CreateFile( path , 0 , FILE_SHARE_READ , NULL , OPEN_EXISTING , FILE_ATTRIBUTE_NORMAL , NULL );
	...
	{
		HandlePtr handle = GetFileAttributeHandle( FilePath , Err );

        if ( Err ) *Err = GetLastError();

		if ( handle.Get() == INVALID_HANDLE_VALUE ) return 0;		

		return GetSize( handle.Get() , Err );
	}
...
namespace USB
{
	CString GetLastDriverName( DWORD* Error )
	{
        ...
        HighOrderVector<CString> Tokens = StrUtil::Tokenizer( Driver , _T('\0') );
        ...
        return (Ret == Tokens.end()) ? CString() : (*Ret);
	}
};