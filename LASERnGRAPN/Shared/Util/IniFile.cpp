...
BOOL CIniFile::WriteTrueFalse(LPCTSTR lpSection, LPCTSTR lpKey, BOOL Value)
{
	return WriteString(lpSection, lpKey, Value ? _T("TRUE") : _T("FALSE"));
}
...
BOOL CIniFile::IsEmptyValue(LPCTSTR lpSection, LPCTSTR lpKey)
{
	return GetString(lpSection, lpKey).IsEmpty();
}
...
BOOL CIniFile::GetTrueFalse(LPCTSTR lpSection, LPCTSTR lpKey, BOOL Default)
{
	CString Value = GetString(lpSection, lpKey);
	if (Value.IsEmpty()) return Default;

	return (Value.MakeUpper() == _T("TRUE")) ? TRUE : FALSE;
}
